    ...
    fn validate_input(&self, arguments: &serde_json::Value) -> Result<(), McpError> {
        if jsonschema::is_valid(self.input_schema(), arguments) {
            ...
            Err(McpError::invalid_params(
                format!(
                    "Invalid parameters for tool '{}': arguments do not match the expected schema",
                    self.name()
                ),
                None,
            ))
...
mod tests {
    ...
    impl DynamicTool for MockDynamicTool {
        ...
        async fn call(
            ...
            if self.should_fail {
                Err(McpError::internal_error("Mock tool failure", None))
            } else {
    ...
    fn create_mock_tool(name: &str, description: &str, should_fail: bool) -> Box<MockDynamicTool> {
        Box::new(MockDynamicTool {
            name: name.to_string(),
    ...
    #[test]
    fn test_dynamic_tool_validate_input_success() {
        let tool = create_mock_tool("test_tool", "Test tool", false);
        ...
        let result = tool.validate_input(&valid_arguments);
        assert!(
            result.is_ok(),
            "Validation should succeed for valid arguments"
        );
    }

    #[test]
    fn test_dynamic_tool_validate_input_failure() {
        let tool = create_mock_tool("test_tool", "Test tool", false);
        ...
        let result = tool.validate_input(&invalid_arguments);
        assert!(
            result.is_err(),
            "Validation should fail for invalid arguments"
        );
        ...
        if let Err(error) = result {
            assert!(
                error
                    .to_string()
                    .contains("Invalid parameters for tool 'test_tool'")
            );
        }
    }

    #[test]
    fn test_dynamic_tool_validate_input_wrong_type() {
        let tool = create_mock_tool("test_tool", "Test tool", false);

        // Wrong type for message field (should be string, not number)
        let invalid_arguments = json!({
            "connection_id": "test_conn",
            "message": 123
        });
        ...
        assert!(
            result.is_err(),
            "Validation should fail for wrong argument types"
        );
    }

    #[test]
    fn test_dynamic_tool_validate_input_extra_fields() {
        let tool = create_mock_tool("test_tool", "Test tool", false);

        // Extra fields should be allowed by default (unless schema is strict)
        let arguments_with_extra = json!({
            "connection_id": "test_conn",
            "message": "Hello world",
            "extra_field": "this should be ignored"
        });

        let result = tool.validate_input(&arguments_with_extra);
        assert!(
            result.is_ok(),
            "Validation should succeed even with extra fields"
        );
    }

    #[test]
    fn test_validate_input_integration_with_schema_variations() {
        // Test with different schema types
        let schema_with_numbers = json!({
            "type": "object",
            "properties": {
        ...
        let tool = Box::new(MockDynamicTool {
            name: "number_tool".to_string(),
            description: "Tool with numbers".to_string(),
            schema: schema_with_numbers,
            should_fail: false,
        });

        // Valid numeric arguments
        let valid_numeric = json!({
            "connection_id": "test",
            "count": 5,
            "factor": 1.5
        });
        assert!(tool.validate_input(&valid_numeric).is_ok());

        // Invalid: negative count (violates minimum constraint)
        let invalid_negative = json!({
            "connection_id": "test",
            "count": -1
        });
        assert!(tool.validate_input(&invalid_negative).is_err());
        ...
        assert!(tool.validate_input(&invalid_type).is_err());
    }
}
