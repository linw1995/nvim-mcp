...
/// Prefers XDG_RUNTIME_DIR, then TMPDIR, falls back to /tmp.
fn get_socket_dir() -> String {
    if cfg!(target_os = "windows") {
        return std::env::var("TEMP").unwrap_or_else(|_| "C:\\temp".to_string());
    }
...
/// Get all directories to search for sockets.
/// Includes both the preferred dir and /tmp as legacy fallback, deduplicated.
fn get_socket_search_dirs() -> BTreeSet<String> {
    BTreeSet::from([get_socket_dir(), "/tmp".to_string()])
}
